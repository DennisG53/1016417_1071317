...
{
    /// <summary>
    /// El árbol está armado en nodos que tienen estas propiedades
    /// </summary>
    public class Nodo
    {
        public int[] valores;
    ...
    public class Lista
    {
        ...
        public static List<int> ingresados;


        public Lista()
        {
            //ingresados = new List<Integer>();
            ingresados = new List<int>();
        }

        public bool buscar(int valor)
        {
            bool esta = false;
            for (int i = 0; i < ingresados.Count() && !esta; i++)
            {
                if (ingresados[i] == valor)
                {
                    esta = true;
    ...
    public class Raiz
    {
        ...
        public void split(Nodo nodo)
        {
            ...
            if (nodo.padre != null)
            { // luego del split y asignar los hijos (hijoIzq, hijoDer), subir el valor al padre
                bool subido = false;
        ...
        { //elimina de la lista el valor y vuelve a crear el arbol
            ...
            int j = 0;
            for (int i = 0; i < Lista.ingresados.Count() && !encontrado; i++)
            {
                if (Lista.ingresados[i] == valor)
                {
                    encontrado = true;
                    j = i;
                }
            ...
            {
                Console.WriteLine("El valor a eliminar no se encuentra en el arbol B");
                //MessageBox.Show("El valor a eliminar no se encuentra en el arbol B");
                //System.out.println("El valor a eliminar no se encuentra en el arbol B");
            }
            List<int> auxiliar = Lista.ingresados;
            //ArrayList<Integer> auxiliar = Lista.ingresados;

            Lista.ingresados = new List<int>();
            //Lista.ingresados = new ArrayList<Integer>();

            primerNodo = new Nodo();
            primerNodo.tengoHijos = false;
            for (int k = 0; k < auxiliar.Count(); k++)
            {
                //********************MODIFICACION DUDOSA QUE HICE*******************
                int y = auxiliar[k];
                insertar(y);
        ...
        {
            bool esta = false;
            for (int i = 0; i < Lista.ingresados.Count() && !esta; i++)
            {
                if (Lista.ingresados[i] == valor)
                {
                    esta = true;
                    Console.WriteLine("El elemento buscado si se encuentra en el arbol B");
                    //MessageBox.Show("El elemento buscado si se encuentra en el arbol B");
                    //System.out.println("El elemento buscado si se encuentra en el arbol B");
                    return esta;
                }
            }
            Console.WriteLine("El elemento buscado no se encuentra en el arbol B");
            //MessageBox.Show("El elemento buscado no se encuentra en el arbol B");
